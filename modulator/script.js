...
let current_css = (noise) => {
	...
	border: ${border()}px solid rgba(255,255,255,.4);
...
let preset_1 = () => {
	...
	r_x.set(0)
	r_z.set(0.08)
	opacity_offset.set(0.3)
	blur_offset.set(2.41)
	letter_spacing.set(44)
	border_radius.set(100)
	border.set(1)
}
...
let text_box = (value, setter) => {
	return html`
		input [oninput=${(e) => setter(e.target.value)} placeholder=${text_already}]
	`
}
